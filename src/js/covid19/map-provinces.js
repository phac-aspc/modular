...
var currentRegion, currentDate, currentDateISC, currentDateLab, indexDate, timeData, timeData2, timeDataISC, timeDataISC2, timeDataLab, timeDataLab2, datesArray, datesArrayISC, datesArrayTests, nestedData, nestedDataISC;
...
let queryStringOptions = {
	"hr": {
		"num": ["total", "deaths", "total_last14", "deaths_last14", "total_last7", "deaths_last7"],
		"rate": ["total", "deaths", "total_last14", "deaths_last14", "total_last7", "deaths_last7"]
	},
	"pt": {
...
let displayOptions = function(map,measure){
		...
		if(d3.select("#dropdownType4").node().value == "pt"){
                d3.select("#map").style("display", "");
                d3.select("#trendData").style("display", "");
                d3.select("#figure1-PT").style("display", "");
                d3.select("#map2").style("display", "none");
                d3.select("#trendData2").style("display", "none");
                d3.select("#figure1-HR").style("display", "none");
                // $("#animateMap").removeAttr('disabled')
            }
            else{
                
                d3.select("#map2").style("display", "");
                d3.select("#trendData2").style("display", "");                
                d3.select("#figure1-HR").style("display", "");

                d3.select("#map").style("display", "none");
                d3.select("#trendData").style("display", "none");
                d3.select("#figure1-PT").style("display", "none");
...
let updateSmallMultipleSelectOptions = function(changedDropdown, curStat, curMeasure){
	let newMeasure = $("#dropdownType6").val()
	let newStat = $("#dropdownType5").val()
	// if the PT/HR dropdown changes, we need to make sure that the current measure exists, otherwise set it to the first/default
	// example -> if on PT/avg and switching to HR, it should become HR/num, furthermore, we will need to check it the current stat exists in the new measure and repeat the same idea as 
	...
	// This series of if statements will make sure measure/stat are good values 
			...
			//PT/HR
			if(Object.keys(smallMultiplesOptions["pt"]).indexOf(newStat) != -1){
				curStat = newStat
			}
			else{
				curStat = Object.keys(smallMultiplesOptions["pt"])[0]
			}
			if(smallMultiplesOptions["pt"][curStat].indexOf(newMeasure) != -1){
				curMeasure = newMeasure;
			}
			else{
				curMeasure = smallMultiplesOptions["pt"][curStat][0]
			}
	...
	if(changedDropdown == null || changedDropdown.attr("id") == 'dropdownType5'){
		displaySmallMultipleOptions(curStat)
	}
	...
	//set dropdown values
	$("#dropdownType5").val(curStat)
	$("#dropdownType6").val(curMeasure)
	
	
}
let displaySmallMultipleOptions = function(stat){
		d3.selectAll("#dropdownType6 option").style("display", "none")
		
		smallMultiplesOptions["pt"][stat].forEach(function(d,i){
			d3.select("#dropdownType6 option[value='"+d+"']").style("display", "").attr("disabled", null)
		})
...
function buildMap(mapJSON) {
	...
	function buildMap2(mapJSON) {
		...
		const repatriated = d3.select("#mapGroup").datum(nestedDataISC[98][0]).append("g")
			...
			.on("mouseover", function(d) {
				...
				d3.select("#textarea").style("margin-top", "20px").style("height", "295px");
		...
		d3.selectAll(".backgroundRepatriated, #repatriated, .textRepatriated, .circleRepatriated").style("display", "none");
		...
		const mapCanada = d3.select("#mapGroup").datum(nestedData[1][0]).append("g")
			...
			.on("mouseover", function(d) {
				...
				//d3.select("#textarea").style("margin-top", "128px").style("height", "185px");
		...
		if (iframeElementMap == null) {
			let queryStringValues = getParams(window.location.href);
			if(queryStringValues.redir == 1){
				d3.select("#archived").style("display", "block")
			}
			...
			d3.select("#txtTotal").text(generateTxtTotal(currentRegion.trend[currentDate][0][typeCases],"1",typeCases));
			...
			d3.select('.CanadaTextValue').each(function(d) {
				d3.select(this).transition().duration(600).tween("text", function() {
					...
					if (language == "en") {
						...
						i = d3.interpolateNumber(that.text().replace(/ /g, "").replace(",", "."), newVal);
						return function(t) { that.text(format(i(t))); };
					}
				});
			});
	...
	$('#dropdownType1, #dropdownType3, #dropdownType4').on('change', function() {
		...
		// }
		// else if ($('#dropdownType3').val() == "avg") {
		// 	$('#dropdownType1 option').css("display","none");
		// 	$('#dropdownType1 option').prop("disabled","true");
		// 	$('#dropdownType1 option[value="tests_last7"]').css("display", "");
		// 	$('#dropdownType1 option[value="tests_last7"]').prop("disabled", "");
		// 	$('#dropdownType1 option[value="ratetests_last7"]').css("display", "");
		// 	$('#dropdownType1 option[value="ratetests_last7"]').prop("disabled", "");
		...
		d3.selectAll(".numArticle").text(function() {
			...
			if (language == "en") {
				...
				// 	return "from";
				// }
				else if ((typeCases == "numtests") || (typeCases == "ratetests") || (typeCases == "numdeaths_last7") || (typeCases == "ratedeaths_last7") || (typeCases == "numdeaths_last14") || (typeCases == "ratedeaths_last14")) {
					return "for";
				}
				else {
					return "of";
	...
	function updateMapNotes(){
		let pthrDD = $("#dropdownType4").val(); 
				
		if(pthrDD == "hr"){
			if(language == "en"){
				var t = document.getElementById("mapNotesText");
				t.innerHTML = "Data presented by health region may differ from those presented elsewhere due to differences in denominators, reporting times, or data adjustments. Adjustments may be made to account for missing data or the receipt of new information.";
			}else{
				var t = document.getElementById("mapNotesText");
				t.innerHTML = "Les données présentées par région sanitaire peuvent différer de celles présentées ailleurs à cause de variations dans les dénominateurs et les heures de rapports, ou du rajustement des données. Un rajustement peut être effectué pour tenir compte de données manquantes ou de la réception d’information nouvelle.";
			}
		}else{
		...
		if(language == "en"){
			...
			var parseTime3 = d3.timeParse("%Y-%m-%d %H:%M");
			var parseTime2 = d3.timeParse("%d-%m-%Y %H:%M");		
			$(".updateDateLab").text(d3.timeFormat("%B %-d, %Y")(d3.timeParse("%d-%m-%Y")(currentDate)));
			$(".updateDate").text(d3.timeFormat("%B %-d, %Y")(d3.timeParse("%d-%m-%Y")(datesArray[datesArray.length - 1])));
			$(".updateTime").text(formatTime(parseTime2(datesArrayTests[datesArrayTests.length - 1]+" 17:00")));
			//$(".updateTime").text(d3.timeFormat("%B %-d, %Y")(d3.timeParse("%d-%m-%Y")(datesArray[datesArray.length - 1])));
		}else{
					    var formatTime = d3.timeFormat("%-d %B %Y, %-H h HNE");
			var parseTime3 = d3.timeParse("%Y-%m-%d %H:%M");
			var parseTime2 = d3.timeParse("%d-%m-%Y %H:%M");
			$(".updateDateLab").text(d3.timeFormat("%-d %B %Y")(d3.timeParse("%d-%m-%Y")(currentDate)));
			$(".updateDate").text(d3.timeFormat("%-d %B %Y")(d3.timeParse("%d-%m-%Y")(datesArray[datesArray.length - 1])));
			//$(".updateTime").text(d3.timeFormat("%-d %B %Y")(d3.timeParse("%d-%m-%Y")(datesArray[datesArray.length - 1])));
			$(".updateTime").text(formatTime(parseTime2(datesArrayTests[datesArrayTests.length - 1]+" 17:00")));
		}
	}
	
	
	function updateSmallMultiplesNotes(){
		if(typeCasesSM == "numtests_last7" || typeCasesSM == "avgtests_last7" || typeCasesSM == "avgratetests_last7" || typeCasesSM == "avgpositivity_last7"){
			if(language == "en"){
				var t2 = document.getElementById("smallMultipleNotesText")
				t2.innerHTML = "<ol type='a'><li>The 7-day moving average is the total of the daily numbers for the previous 7 days (up to and including the day of the last update), divided by the number of days for which data is available. We go back and update the moving averages as provinces and territories submit more data. We calculate the national 7-day moving average by summing the 7-day moving average from the provinces and territories then dividing by the national population if a rate is calculated.</li></ol>";
			}else{
				var t2 = document.getElementById("smallMultipleNotesText")
				t2.innerHTML = "<ol type='a'><li>La moyenne mobile sur 7 jours est le total des chiffres quotidiens des 7 jours précédents (jusqu’au jour de la dernière mise à jour inclusivement) divisé par le nombre de jours pour lesquels des données sont disponibles. Nous mettons à jour les moyennes mobiles à mesure que les provinces et les territoires fournissent de nouvelles données. Nous calculons la moyenne mobile nationale sur 7 jours en additionnant les moyennes mobiles sur 7 jours des provinces et territoires, puis en divisant par la population nationale si un taux est calculé.</li></ol>";
			}
		}else{
			if(language == "en"){
				var t2 = document.getElementById("smallMultipleNotesText")
		...
		d3.select("#map svg .bottomLeg text")
			.text(function() {
				if (language == "en") {
					...
					if (typeCases == "numtoday") {
						return "Note: When displaying the number of new cases, data is only available for a complete day." //On First Nations reserves numbers reported to Indigenous Services Canada are counted in the provincial and territorial total numbers.";
					}
					else if (typeCases == "numtests") {
						return "Note: Provincial/territorial (PT) data reported on their websites should be used if there are discrepancies. This can be due to lags, differing reporting cut-offs, or changes in lab testing criteria." //On First Nations reserves numbers reported to Indigenous Services Canada are counted in the provincial and territorial total numbers.";
					}
					else if (typeCases == "numtotal") {
						return "Note: The total number includes publicly reported confirmed and probable cases." //On First Nations reserves numbers reported to Indigenous Services Canada are counted in the provincial and territorial total numbers.";
					}
					else if (typeCases == "numrecover") {
						return "Note: On July 17, there was an increase in the number of recovered cases in Quebec due to a revision in how they define these cases." //On First Nations reserves numbers reported to Indigenous Services Canada are counted in the provincial and territorial total numbers."
					}
					else{
						//return "Note: On First Nations reserves numbers reported to Indigenous Services Canada are counted in the provincial and territorial total numbers. ";
					}
				}
				else {
						
					if (typeCases == "numtoday") {
						return "Remarque : Lorsque le nombre de nouveaux cas est affiché, les données sont seulement disponibles pour une jour complète." //Dans les réserves des Premières Nations, les chiffres signalés à Services aux Authochtones Canada sont comptés dans les chiffres provinciaux et territoriaux.";
					}
					else if (typeCases == "numtests") {
						return "Remarque : Les données provinciales / territoriales (PT) signalées sur les sites Web des provinces et territoires être utilisées en cas d’écarts dans les données. Ces écarts peuvent être dûs à des retards, des dates de rapports différentes ou des changements dans les critères relatifs aux essais en laboratoire." //Dans les réserves des Premières Nations, les chiffres signalés à Services aux Authochtones Canada sont comptés dans les chiffres provinciaux et territoriaux.";
					}
					else if (typeCases == "numtotal") {
						return "Remarque : Le nombre total comprend les cas confirmés et probables rapportés publiquement." //Dans les réserves des Premières Nations, les chiffres signalés à Services aux Authochtones Canada sont comptés dans les chiffres provinciaux et territoriaux.";
					}
					else if (typeCases == "numrecover") {
						...
						//return "Remarque : Dans les réserves des Premières Nations, les chiffres signalés à Services aux Authochtones Canada sont comptés dans les chiffres provinciaux et territoriaux.";
					}
				}
			})
			.call(wrap, 395)
	...
	function drawTrend() {
		...
		svg.append("text")
			...
			.text(function(d) {
				if (language == "en") {
					if(typeCases == "avgratetests_last7") {
						return txtTypeMod + " of rate of tests (last 7 days)";
					}else{
					...
					if(typeCases == "avgratetests_last7") {
						return txtTypeMod + " du taux de tests (derniers 7 jours)";
					}else{
...
updateSmallMultipleSelectOptions(null, $('#dropdownType5 option:selected').val(), $('#dropdownType6 option:selected').val())
	$('#dropdownType6').on('change', function() {
		...
		typeModSM = $('#dropdownType5 option:selected').val();
		typeCasesSM = $('#dropdownType5 option:selected').val() + $('#dropdownType6 option:selected').val();
		...
		updateSmallMultiplesNotes();
	})

	$('#dropdownType5').on('change', function() {
		//Disable choices where necessary
updateSmallMultipleSelectOptions(d3.select(this), $('#dropdownType5 option:selected').val(), $('#dropdownType6 option:selected').val())
		typeModSM = $('#dropdownType5 option:selected').val();
		typeCasesSM = $('#dropdownType5 option:selected').val() + $('#dropdownType6 option:selected').val();
		txtTypeCasesSM = $('#dropdownType6 option:selected').text();
		txtTypeModSM = $('#dropdownType5 option:selected').text();
	...
	function drawSmallMultipleTrends(logSwitch, relativeSwitch) {
		...
		smallMultipleSVG.append("text")
			...
			.attr("font-size",function() {
				if (typeCasesSM == "avgratetests_last7" || typeCasesSM == "avgpositivity_last7") {
					return "9px";
				} else if (typeCasesSM == "numtests_last7" || typeCasesSM == "avgtests_last7") {
					return "12px";
				} else {
					return "14px";
				}
	...
	function updateSmallMultipleTrends(logSwitch, relativeSwitch) {
		...
		smallMultipleSVG.selectAll(".line").each(function(d) {
			...
			d3.select(this.parentNode).select(".trendXSM")
				.transition()
				.duration(600)
				.call(xaxis);	
			
			d3.select(this.parentNode).select(".trendXSM").selectAll("text")
			.attr("y", 9)
			.attr("x", -21)
			.attr("dy", ".35em")
			.attr("transform", "rotate(-45)");
		...
		smallMultipleSVG.selectAll(".yAxisSMLabel")
			.text(function() {
				if (language == "en") {
					return txtTypeModSM + " of " + txtTypeCasesSM.toLowerCase();
				}
	...
	function generateTxtTotal(numTxtTotal, id, typeCases) {
		...
		let formatTxtTotal = d3.format(",d");
		if(typeCases == "avgpositivity_last7"){
			formatTxtTotal = d3.format("0.1f");	
		}
		if (id != "98") {
			if (exceptions[id][0]["Notes Map"] != "" && exceptions[id][0][typeCasesExceptions] == "TRUE") {
				if (language == "en") {
					numTxtTotalFormatted =formatTxtTotal(numTxtTotal) + "<sup>" + notesSymbols[currentNoteNumber] + "</sup>";
				}
				else {
					numTxtTotalFormatted = (formatTxtTotal(numTxtTotal) + "<sup>" + notesSymbols[currentNoteNumber]).replace(",", " ") + "</sup>";
				}
				numberOfNotes++;
			}
			else {
				if (language == "en") {
					if(numTxtTotal == null){
						numTxtTotalFormatted = "not available";
					}else{
						numTxtTotalFormatted = formatTxtTotal(numTxtTotal);
					}
				}
				else {
					if(numTxtTotal == null){
						numTxtTotalFormatted = "sans objet";
					}else{
						numTxtTotalFormatted = formatTxtTotal(numTxtTotal);
					}
				}
		...
		//console.log(numTxtTotalFormatted,formatTxtTotal(numTxtTotal),numTxtTotal);
	...
	function createTable() {
		...
		dataRow.append("td")
			...
			.html(function(d) {
				if (timeDataLab[d.key][currentDateLab]) {
	...
	function createTableIframe() {
		...
		dataRow.append("td")
			...
			.text(function(d) {
				if (timeData[d.key][currentDate]) {
					if(timeDataLab[d.key][currentDateLab][0].numtests == null){
						if(language == "en")
							return "N/A"
						else
							return "s.o."
					}else{
						return d3.format(",d")(timeData[d.key][currentDate][0].numtests);
					}
				}
				else {
					if(language == "en")
			...
			.text(function(d) {
				if (timeDataLab[d.key][currentDateLab]) {
					if(timeDataLab[d.key][currentDateLab][0].avgtests_last7 == null){
						...
						return d3.format(",d")(timeDataLab[d.key][currentDateLab][0].avgtests_last7);
					}
				}
				else {
					if(language == "en")
			...
			.text(function(d) {
				if (timeDataLab[d.key][currentDateLab]) {
					if(timeDataLab[d.key][currentDateLab][0].avgratetests_last7 == null){
						...
						return d3.format(",d")(timeDataLab[d.key][currentDateLab][0].avgratetests_last7);
					}
				}
				else {
					if(language == "en")
			...
			.text(function(d) {
				if (timeDataLab[d.key][currentDateLab]) {
					if(timeDataLab[d.key][currentDateLab][0].avgpositivity_last7 == null){
						...
						return d3.format(",d")(timeDataLab[d.key][currentDateLab][0].avgpositivity_last7);
					}
				}
				else {
					if(language == "en")
