...
Promise.all(promises).then(function(values) {
	if(urlProduct == "cov"){
    	...
    	processData(0,0,0,0,0,0,values[10],values[6],0,values[7],values[8]);
	}
}).then(function(){
	if(urlProduct == "cov"){
	    buildKeyUpdates();
	...
	if(urlProduct == "cov"){
	 ...
	 //   }); 
	  ...
	  //  mapDataDist = d3.nest()
	    ...
	    date_numtotal_dosesdistributed = parseTime(datesArrayDist[datesArrayDist.length-1]);
	    d3.selectAll(".numtotal_dosesdistributed_txt").text(numberFormat(current_numtotal_dosesdistributed));
...
function buildTables() {
	...
	if(urlProduct == "cov"){
		...
		dataDistributed.each(function(value,index){
			value.each(function(value2,index2){
