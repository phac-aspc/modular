...
Promise.all(promises).then(function(values) {
	if(urlProduct == "cov"){
    	processData(values[2],values[1],values[3],values[4],values[5],values[0],0,0,0,0,values[8],values[9],0);
	}else{
    	processData(0,0,0,0,0,0,values[10],values[6],0,values[7],values[8],0,values[11]);
	}
}).then(function(){
	if(urlProduct == "cov"){
	    buildKeyUpdates();
	 ...
	 //   }); 
	  ...
	  //  dataDistributed = d3.nest()
	    ...
	    dataDistributed2 = d3.nest()
			.key(function(d) { return d.pruid; }).sortKeys(d3.ascending)
			.key(function(d) { return formatTimeAdminDist(d.date); }).sortKeys(d3.ascending)
	  ...
	  //  mapDataDist = d3.nest()
	    ...
	    data_forecast.forEach(function(d){
	    	d.date = parseTime(d.forecast_week);
        ...
        if(typeDropdownVal == "_administered"){
		    if(language == "en"){
	        	...
	        	$(".updateDateAdmin").text(formatTimeAdmin(parseTimeAdmin(updateDateAdmin)).replace("PM", "pm").replace("AM", "am"));
		    }else{
		    	formatTimeAdmin = d3.timeFormat("%-d %B %Y, %-H h (Heure de l'Est)");
	        	$(".updateDateAdmin").text(formatTimeAdmin(parseTimeAdmin(updateDateAdmin)));
		    }
        }else{
	    	if(language=="en"){
		    	formatTimeAdmin = d3.timeFormat("%B %-d, %Y, %-I %p EST");
				$(".updateDateAdmin").text(formatTimeAdmin(date_numtotal_dosesdistributed).replace("PM", "pm").replace("AM", "am"));
	    	}else{
		    	formatTimeAdmin = d3.timeFormat("%-d %B %Y, %-H h (Heure de l'Est)");
	        	$(".updateDateAdmin").text(formatTimeAdmin(date_numtotal_dosesdistributed));
	    	}
        }
...
// 		createTable();
		if(typeDropdownVal == "_administered"){
		    ...
		    d3.selectAll(".updateDateAdminDist").text(function(){
		    	if(language=="en"){
			    	formatTimeAdmin = d3.timeFormat("%B %-d, %Y, %-I %p EST");
		        	$(".updateDateAdmin").text(formatTimeAdmin(parseTimeAdmin(updateDateAdmin2)).replace("PM", "pm").replace("AM", "am"));
					return d3.timeFormat("%B %e, %Y")(parseTime(asofdate_numtotal_dosesadministered));
		    	}else{
			    	formatTimeAdmin = d3.timeFormat("%-d %B %Y, %-H h (Heure de l'Est)");
		        	$(".updateDateAdmin").text(formatTimeAdmin(parseTimeAdmin(updateDateAdmin2)));
					return d3.timeFormat("%d %B %Y")(parseTime(asofdate_numtotal_dosesadministered));
		    	}
		    });
			currentRegion = dosesadministered[0].values;
		...
		mapDataDist.each(function(value,index){
			value.each(function(value2,index2){
				if(value2.key != "99"){ // we could consider leaving 99 (federal allocation) in the table
					...
					currentRow.append("td")
					.attr("data-order", function(){
						return d3.timeFormat("%s")(formatTime(value2[0].date, "YYYY-MM-DD"));
					})
					.attr("data-sort", formatTime(value2[0].date, "YYYY-MM-DD"))
					.text(formatTime(value2[0].date, "YYYY-MM-DD"));
					currentRow.append("td").text(pruid2prov(value2[0].pruid));
