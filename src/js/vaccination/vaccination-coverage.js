...
var getURLProduct = function(url) {
	if((url.search("administres") > -1)||url.search("administration") > -1)
		return "admin";
	return "cov";
};
...
var csvfiles = [
    ...
    // '/src/data/covidLive/vaccination-distribution.csv',
...
Promise.all(promises).then(function(values) {
	if(urlProduct == "cov"){
    	processData(values[2],values[1],values[3],values[4],values[5],values[0],0,0,0,0,values[8],values[9]);
	}else{
    	processData(0,0,0,0,0,0,0,values[6],0,values[7],values[8]);
	}
}).then(function(){
	if(urlProduct == "cov"){
	    buildKeyUpdates();
...
//Process Data
var nestedData_byVac2, keyPop_nestedData, overall_nestedData, timeArray, data_overall, data_keyPop, current_numtotal_dosesdistributed, date_numtotal_dosesdistributed, current_numtotal_dosesadministered, current_numtotaldelta_all_administered, date_numtotal_dosesadministered, asofdate_numtotal_dosesadministered, nestedData_byAge, nestedData_bySex, nestedData_byVac, datesArray, datesArray2, datesArrayAdmin, datesArrayAdmin2, asofdatesArrayAdmin, asofdatesArrayAdmin2, mapData, mapDataDist, mapDataAdmin, dosesadministered, dosesadministered2, dosesadministered3, nestedData_bySexOnly, nestedData_bySexOnlyDenoms, covlineData, updateDateCov;
function processData(data_overall,data_keyPop,data_byAge,data_denoms,data_byVaccine,data_map,data_distributed,data_map_admin,data_map_dist,updateDateAdmin,notes,updateDateCov){
	...
	if(urlProduct == "cov"){
	    ...
	    .key(function(d) { return d.pop_group })
	  ...
	  //  mapDataDist = d3.nest()
			...
			// .key(function(d) { if (datesArrayAdmin.indexOf(d["week_end"]) < 0) { datesArrayAdmin.push(d["week_end"]); } return d["week_end"];	}).sortKeys(d3.ascending)
			// .map(data_distributed);
		
	  //  current_numtotal_dosesdistributed = +mapDataDist.get("1").get(datesArrayAdmin[datesArrayAdmin.length-1])[0].numtotal_all_distributed;
	  //  date_numtotal_dosesdistributed = parseTime(datesArrayAdmin[datesArrayAdmin.length-1]);
	    ...
	    nestedData_byVac = d3.nest()
	        ...
	        .rollup(function(leaves){
	        	return {
	          //      numtotal_partially: leaves[0]["numtotal_partially"],
	        		// numtotal_fully: leaves[0]["numtotal_fully"],
	        		// numtotal_atleast1dose: leaves[0]["numtotal_atleast1dose"],
	        		// prop_partially: leaves[0]["prop_partially"],
	        		// prop_fully: leaves[0]["prop_fully"],
	        		// prop_atleast1dose: leaves[0]["prop_atleast1dose"],
	        	    numtotal_fully: d3.sum(leaves,function(leaf){
	                	return +leaf["numtotal_fully"];
	                	}),
	                numtotal_atleast1dose: d3.sum(leaves,function(leaf){
	 ...
	 //   }); 
	    ...
	    if(language == "en"){
        	...
        	$(".updateDateAdmin").text(formatTimeAdmin(parseTimeAdmin(updateDateAdmin["columns"][0])).replace("PM", "pm").replace("AM", "am"));
	    }else{
	    	formatTimeAdmin = d3.timeFormat("%-d %B %Y, %-H h (Heure de l'Est)");
        	$(".updateDateAdmin").text(formatTimeAdmin(parseTimeAdmin(updateDateAdmin["columns"][0])));
	    }
         
		formatTime = d3.timeFormat("%Y-%m-%d");
		$(".dateModified").text(formatTime(parseTimeAdmin(updateDateAdmin["columns"][0])));
...
function buildKeyUpdates() {
    ...
    //Pop group - total
    d3.selectAll(".prop_atleast1dose_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].prop_atleast1dose,"prop"));
    d3.selectAll(".prop_fully_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].prop_fully,"prop"));
    d3.selectAll(".prop_partially_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].prop_partially,"prop"));
    d3.selectAll(".numtotal_atleast1dose_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].numtotal_atleast1dose,"num"));
    d3.selectAll(".numtotal_fully_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].numtotal_fully,"num"));
    d3.selectAll(".numtotal_partially_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].numtotal_partially,"num"));
    d3.selectAll(".propweekdelta_atleast1dose_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].propweekdelta_atleast1dose,"prop"));
    d3.selectAll(".propweekdelta_fully_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].propweekdelta_fully,"prop"));
    d3.selectAll(".propweekdelta_partially_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].propweekdelta_partially,"prop"));
    d3.selectAll(".numweekdelta_atleast1dose_txt").text(generateTxt(overall_nestedData[timeArrayEndPos].values[popGroupTotal].values[0].numweekdelta_atleast1dose,"num"));
...
// 			d3.select("#descText2").text(timeValue);
    ...
    function vaccineCoverageMap(mapJSON) {
		...
		//Add text to circle el backgrounds
...
// 		$('#dropdownType1').val()=="recover" ? $('#dropdownType3 option[value="rate"]').prop("disabled","true") : $('#dropdownType3 option[value="rate"]').prop("disabled","");
		...
		numPropDropdownVal2 = $('#map-ddl-1').val();
		// if(numPropDropdownVal == undefined){
		// 	numPropDropdownVal = "num";
		// }
		if(numPropDropdownVal2.slice(0,3) == "num"){
			numPropDropdownVal = "num";
...
// 		createTable();
	...
	function color(value) {
		if(urlProduct == "cov"){
			if(vaccineDropdownVal == "_all" || vaccineDropdownVal == "_pfizerbiontech" || vaccineDropdownVal == "_moderna"){
				...
				if(numPropDropdownVal == "num") {
					if(numPropDropdownVal2 == "numtotal"){
						...
						else if (value >= 40000) return colorScale["3"];
				...
				if(numPropDropdownVal == "num") {
					if(numPropDropdownVal2 == "numtotal"){
						if (value >= 3000000) return colorScale["1"];
					...
					if(numPropDropdownVal2 != "propweekdelta"){
						if (value >= 70.00) return colorScale["1"];
						else if (value >= 60.00) return colorScale["2"];
						else if (value >= 50.00) return colorScale["3"];
						else if (value >= 40.00) return colorScale["4"];
						else if (value >= 0) return colorScale["5"]
						else return colorScale["No data"];
					}else{
						if (value >= 10.00) return colorScale["1"];
			...
			if (value >= 4000000) return colorScale["1"];
			else if (value >= 1000000) return colorScale["2"];
			else if (value >= 500000) return colorScale["3"];
			else if (value > 100000) return colorScale["4"];
			else if (value >= 0) return colorScale["5"]
			else return colorScale["No data"];
		}
	}
		...
		if(urlProduct == "cov"){
			if(vaccineDropdownVal == "_all" || vaccineDropdownVal == "_pfizerbiontech" || vaccineDropdownVal == "_moderna"){
				...
				if(numPropDropdownVal == "num"){
					if (language == "en") {
						...
						if (i == 0) {
							...
							return "40 000 à 99 999";
						}
						else if (i == 3) {
							return "20 000 à 39 999";
						}
						else if (i == 4) {
							return "0 à 19 999";
						}
				...
				if(numPropDropdownVal == "num"){
					if (language == "en") {
						if (i == 0) {
							return "3,000,000 and higher";
						}
						else if (i == 1) {
							return "1,000,000 to 2,999,999";
						}
						else if (i == 2) {
						...
						if (i == 0) {
							return "3 000 000 et plus";
						}
						else if (i == 1) {
							return "1 000 000 à 2 999 999";
						}
						else if (i == 2) {
					...
					if (language == "en") {
						if (i == 0) {
							return "70.0% and higher";
						}
						else if (i == 1) {
							return "60.0% to 69.99%";
						}
						else if (i == 2) {
							return "50.0% to 59.99%";
						}
						else if (i == 3) {
							return "40.0% to 49.99%";
						}
			...
			if (language == "en") {
				if (i == 0) {
					return "4,000,000 and higher";
				}
				else if (i == 1) {
					return "1,000,000 to 3,999,999";
				}
				else if (i == 2) {
				...
				if (i == 0) {
					return "4 000 000 et plus";
				}
				else if (i == 1) {
					return "1 000 000 à 3 999 999";
				}
				else if (i == 2) {
...
function draw_fig_agesex(_option,_vaxStat,pruid,_update){
   ...
   // }
	...
	data.values.sort(function(a,b) { 
		if(a.key == "All ages"){
			return data.values.length - ageGroups_arr.en.indexOf(b.key);
		}else if(b.key == "All ages"){
			return ageGroups_arr.en.indexOf(a.key) - data.values.length;
		}else{
			return ageGroups_arr.en.indexOf(a.key) - ageGroups_arr.en.indexOf(b.key);
		}
	})
	...
	if(_update){
		...
		d3.select("svg#byAgeGroup").select("g #fig5_y")
			...
			.delay(300)
			.duration(600)
			.call(d3.axisLeft(y).ticks(ticks).tickFormat(function(d){
				if(_option == "prop"){
					if(language == "en"){
						return generateTxt(d,"prop",0);
					}else{
   ...
   //-------------------------------------------------------------------
	...
	let legend = svg.append("g");
			...
			sexSubgroups_arr.forEach(function(value,index){
				legend.append("rect")
						.attr("stroke","none")
						.attr("x", (legendDim.x /3 + ((chartDim.width/5)*index)))
						.attr("y", legendDim.y)
						.attr("width", legendDim.sqr)
						.attr("height",legendDim.sqr)
						.attr("fill",function(){
				...
				legend.append("text")
    		        	...
    		        	.attr("font-size","16px")
    		        	.attr("font-weight","bold")
    		        	.attr("x", (legendDim.x /3 + ((chartDim.width/5)*index) + legendDim.sqr + legendDim.textPadding) )
    		        	.attr("y", (legendDim.y + legendDim.sqr/1.2))
    		        	.text(function(d){
...
// // var formatTime = d3.timeFormat("%b %d '%y");
    ...
    var pathGen = d3.line()
    	.defined(function(d){
            if((d.value[_option+"_"+_vaxStat] === "") || isNaN(d.value[_option+"_"+_vaxStat])){
                return false;
            }else{
                return true;
            }
					...
					legend.selectAll("text")
							...
							.text(function(d){
								if(data.values[d.index].values.length == datesArray2.length){
									return ": "+generateTxt(data.values[d.index].values[dateIndex].value["table_"+_option+"_"+_vaxStat],_option)
								}else{
									for(let z=0 ; z<data.values[d.index].values.length ; z++){
										if(data.values[d.index].values[z].key == d3.timeFormat("%Y-%m-%d")(hoveringDate)){
											return ": "+generateTxt(data.values[d.index].values[z].value["table_"+_option+"_"+_vaxStat],_option)
										}
									}
									return ": "+generateTxt("");
								}
...
// 						})
    ...
    axis.append("text")
            ...
            .attr("transform",function(){
                return "translate("+ -25 +","+ ((trendLineDim.height/2) - 30) +")rotate(-90)";
            })
            .append("tspan")
