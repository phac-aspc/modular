...
// 2021
    ...
    // all
        ...
        //benzo
        //stim
let totalsForLocations = {'QC': 31781, 'MB': 2477, 'ON': 33832, 'NB': 1981, 'NS': 1409, 'AB': 12413, 'SK': 2022, 'BC': 15588, 'NL': 482, 'PE': 212, 'T': 231, 'CA': 102428}

//these 3 are covered by having last years data

//add year and quarter
let lastYearSameQuarter = { "QC": 38189, "MB": 2515, "ON": 38205, "NB": 1354, "NS": 1758, "AB": 14714, "SK": 1947, "BC": 14766, "NL": 587, "PE": 155, "T": 205, "CA": 114395 }


//add year/quarter 
let benzoLastYear = {"ON":867,"AB":272,"QC":1295,"MB":54,"NS":69,"NB":65,"BC":270,"SK":20,"NL":35,"PE":2,"T":4,"CA":2953}
let stimulantLastYear = {"ON":19843,"AB":8727,"QC":20113,"MB":1715,"NS":705,"NB":710,"BC":7600,"SK":1238,"NL":173,"PE":124,"T":145,"CA":61093}
...
let getDataForCanadaByQuarter = function() {
    ...
    byQuarter = canadaData;

    canada.values = byQuarter;

    totalForLocation = totalsForLocations["CA"];
    //console.log(canada)
    return canada;
};
...
let getDataForProvinceByQuarter = function(q, p) {
    ...
    if (q === "All") {
        byQuarter = drugSummary["Quarters"][curType]
    }
    else {
        byQuarter = drugSummary["Quarters"][curType].filter(
            function(data) {
                return data.key === q;
            }
        );
    ...
    // if p == "T" return territories

    byProvince = byQuarter.slice();
    //console.log(p)
    // if (p == "T") {

    //     byProvince = byProvince.slice().filter(
    //         function(data) {
    //             return data.key === "YT" || data.key === "NT" || data.key === "NU";
    //         }
    //     );

    //     let input = [].concat(byProvince[0].values, byProvince[1].values, byProvince[2].values).slice();
    //     // let input = byProvince.slice()[0].values.slice().concat(byProvince.slice()[1].values.slice()).concat(byProvince.slice()[2].values.slice());
    //     console.log(input)
    ...
    //     byProvince = [{ key: "T", values: output }]

    // }
    // else {

        byProvince = byProvince.filter(
            function(data) {
                return data.key === p;
            }
        );
    // }
    console.log(byProvince[0])
    
    byProvince = byProvince[0];
    // this finds the total exhibits
    //console.log(byProvince)

    totalForLocation = totalsForLocations[p];
...
let generatePanCanTableData = function() {
    ...
    panCanData.forEach(function(d, i) {
        let tableRow;
        if (language == "en") {
        ...
        d3.select("#" + id + " .table").append("p").attr("class", "otherNote").text(function() {
            ...
            if (language == "en") {
                if (id == "controlledSubstanceTile")
                    return '* This substance is part of the bar "Other controlled substances" in the graph'

                return '* This substance is part of the bar "Other ' + data.title + '" of the graph'
            }
            else {
                if (id == "controlledSubstanceTile")
                    return '* Cette substance fait partie de la barre "Autres substances controlées" du graphique'

                return '* Cette substance fait partie de la barre "Autres ' + data.title + '" du graphique'
            }
        })
    }
...
let getOpioidDrugTotals = function() {
    ...
    if (language == "en")
        descText = "A total of " + numberFormat(totalCount) + " opioids were identified during that period where " + opioidPercents[curProvince]["fent"] + "% were fentanyl or fentanyl analogues. From all samples containing heroin, " + opioidPercents[curProvince]["fentWithHeroin"] + "% also contained fentanyl.";
    else
        descText = "Un total de " + numberFormat(totalCount) + " opioïdes ont été identifiés au cours de cette période alors que " + opioidPercents[curProvince]["fent"] + " % étaient du fentanyl ou des analogues du fentanyl. De tous les échantillons contenant de l'héroïne, " + opioidPercents[curProvince]["fentWithHeroin"] + " %  contenaient également du fentanyl.";
...
let getControlledSubstanceDrugTotals = function() {
    ...
    if (language == "en")
        percent = Math.abs(100 - ((totalForLocation / lastYearSameQuarter[curProvince]) * 100)).toFixed(0) + "%";
    else
        percent = Math.abs(100 - ((totalForLocation / lastYearSameQuarter[curProvince]) * 100)).toFixed(0) + " %";

    if (totalForLocation < lastYearSameQuarter[curProvince]) {
        if (language == "en")
            increaseDecrease = "decrease"
        else
            increaseDecrease = "diminution"
...
let generateNewSubstanceVisual = function(data) {
    ...
    if (data.data.length > 10) {
        d3.select("#" + data.tileId + " .table").append("p").attr("class", "otherNote").text(function() {
            //console.log(data)
            if (language == "en") {
                return '* This substance is part of the "Other substances" bar of the graph'

            }
            else {
                return '* Cette substance fait partie de la barre "Autres substances" du graphique'
            }
        })
    }
...
let updateTitle = function() {
    ...
    d3.select("#quarter").text(function() {
        if (language == "en")
            return "January to December (Q1-Q4) - 2020";
        else
            return "Janvier à Décembre (T1-T4) - 2020"
...
let generatePanCanadianData = function() {
    ...
    Object.keys(provinceJson).forEach(function(d, i) {
        if (d !== "CA") {
            ...
            curData = getDataForProvinceByQuarter("All", d);
...
let panCanData = [{ "Province": "ON", "Opioids": 9207, "Cannabis": 4192, "Stimulants": 17154, "Benzodiazepines": 1421 }, { "Province": "NL", "Opioids": 55, "Cannabis": 113, "Stimulants": 192, "Benzodiazepines": 14 }, { "Province": "QC", "Opioids": 1015, "Cannabis": 7691, "Stimulants": 15453, "Benzodiazepines": 1288 }, { "Province": "AB", "Opioids": 2753, "Cannabis": 818, "Stimulants": 7285, "Benzodiazepines": 482 }, { "Province": "MB", "Opioids": 453, "Cannabis": 152, "Stimulants": 1744, "Benzodiazepines": 96 }, { "Province": "SK", "Opioids": 233, "Cannabis": 251, "Stimulants": 1239, "Benzodiazepines": 31 }, { "Province": "BC", "Opioids": 6481, "Cannabis": 1201, "Stimulants": 7295, "Benzodiazepines": 730 }, { "Province": "NS", "Opioids": 99, "Cannabis": 289, "Stimulants": 670, "Benzodiazepines": 64 }, { "Province": "NB", "Opioids": 206, "Cannabis": 209, "Stimulants": 1035, "Benzodiazepines": 142 }, { "Province": "T", "Opioids": 29, "Cannabis": 31, "Stimulants": 126, "Benzodiazepines": 6 }, { "Province": "PE", "Opioids": 21, "Cannabis": 21, "Stimulants": 154, "Benzodiazepines": 5 }];
let panCanDataXAxis = {
    "en": { "ON": "ON", "NL": "N.L.", "QC": "QC", "AB": "AB", "MB": "MB", "SK": "SK", "BC": "BC", "NS": "N.S", "NB": "N.B", "T": "Territories", "PE": "P.E.I" },
    "fr": { "ON": "ON", "NL": "T.-N.-L.", "QC": "QC", "AB": "AB", "MB": "MB", "SK": "SK", "BC": "C.-B.", "NS": "N.-É.", "NB": "N.-B.", "T": "Territoires", "PE": "Î.-P.-É." }
}
let panCanClassList = {
    "en": { "ON": "ON", "N.L.": "NL", "QC": "QC", "AB": "AB", "MB": "MB", "SK": "SK", "BC": "BC", "N.S": "NS", "N.B": "NB", "Territories": "T", "P.E.I": "PE" },
    "fr": { "ON": "ON", "T.-N.-L.": "NL", "QC": "QC", "AB": "AB", "MB": "MB", "SK": "SK", "C.-B.": "BC", "N.-É.": "NS", "N.-B.": "NB", "Territoires": "T", "Î.-P.-É.": "PE" }
}
...
let generatePanCanVisual = function() {
    ...
    g.append("g")
        .attr("class", "ygrid")
        .call(d3.axisLeft(y).ticks(null, "s").tickFormat("").tickSize(-width)).selectAll("line").style("opacity", 0.4)
...
let updateCurTable = function() {
    let curData = panCanData.find(function(d) {
