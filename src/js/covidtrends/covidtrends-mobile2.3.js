...
if (qs["source"]) {
	if (qs["source"].toLowerCase() === "weathercan") {
		...
		let filesToLoad = csvfiles.slice(0, 7);
		...
		Promise.all(promises).then(function(values) {
			...
			processVaxUpdateDate(values[3]);
			processFluWatchers(values[4]);
			processMobility(values[5]);
			processWebsites(values[6]);
			processPlacesToHR(values[7]);
			// processLocationToHRUID(values[7])
		}).then(function() {
		...
		Promise.all(promises).then(function(values) {
			...
			processPostalLookup(values[7]);
			processCensusDistrictLookup(values[8]);
		}).then(function() {
			d3.json(HRmap_shapes, function(d) {
				return d;
			}).then(function(data) {
